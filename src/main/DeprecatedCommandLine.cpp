...
const struct option acg_core_options[] = {
...
usage(int err = 1)
{
    ...
    os << "usage: acg-core [OPTIONS]\n"
          ...
          "      --forcescp           Next time acg-core is run, SCP will "
          "start "
...
handleDeprecatedCommandLine(int argc, char* const* argv)
{
    ...
    std::string cfgFile("acg-core.cfg");
    std::string command;
    ...
    int opt;
    while ((opt = getopt_long_only(argc, argv, "c:", acg_core_options,
                                   nullptr)) != -1)
    {
        switch (opt)
        {
