...
void
save(Archive& ar, acg::Upgrades::UpgradeParameters const& p)
{
    ar(make_nvp("time", acg::VirtualClock::to_time_t(p.mUpgradeTime)));
...
template <class Archive>
void
load(Archive& ar, acg::Upgrades::UpgradeParameters& o)
{
    time_t t;
    ar(make_nvp("time", t));
    o.mUpgradeTime = acg::VirtualClock::from_time_t(t);
               ...
               AccountID const& accountID, Asset const& asset, int64_t delta)
{
    ...
    if (iter->second)
    {
        if (!acg::addBalance(*iter->second, delta))
        {
            iter->second.reset();
        }
    }
                                        ...
                                        AbstractLedgerState& ls)
{
    ...
    {
        auto trust = acg::loadTrustLineWithoutRecord(ls, accountID, asset);
...
prepareLiabilities(AbstractLedgerState& ls, LedgerStateHeader const& header)
{
    ...
    for (auto& accountOffers : offersByAccount)
    {
        ...
        auto accountEntry = acg::loadAccount(ls, accountOffers.first);
        if (!accountEntry)
        {
        ...
        for (auto& offerEntry : accountOffers.second)
        {
            ...
            {
                acg::addNumEntries(header, accountEntry, -1);
            }
        ...
        for (auto const& assetLiabilities : liabilities)
        {
            ...
            {
                auto trustEntry =
                    acg::loadTrustLine(ls, accountOffers.first, asset);
                int64_t deltaSelling =
