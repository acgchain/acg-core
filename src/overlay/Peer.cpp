...
void
Peer::recvMessage(StellarMessage const& acgMsg)
{
    if (shouldAbort())
    {
        return;
    ...
    if (Logging::logTrace("Overlay"))
        CLOG(TRACE, "Overlay")
            ...
            << mApp.getConfig().toShortString(
                   mApp.getConfig().NODE_SEED.getPublicKey())
            << ") recv: " << msgSummary(acgMsg)
            << " from:" << mApp.getConfig().toShortString(mPeerID);

    if (!isAuthenticated() && (acgMsg.type() != HELLO) &&
        (acgMsg.type() != AUTH) && (acgMsg.type() != ERROR_MSG))
    {
        CLOG(WARNING, "Overlay")
            << "recv: " << acgMsg.type() << " before completed handshake";
        drop();
        return;
    }

    assert(isAuthenticated() || acgMsg.type() == HELLO ||
           acgMsg.type() == AUTH || acgMsg.type() == ERROR_MSG);
