...
The `get` and `put` commands define subprocesses that `acg-core` will
...
called `s3acg` based on the [AWS S3](https://aws.amazon.com/s3/) storage system:

~~~~
[HISTORY.s3acg]
get="curl http://history.acg.org/{0} -o {1}"
put="aws s3 cp {0} s3://history.acg.org/{1}"
~~~~

In this example, `acg-core` will use the `curl` command to fetch history and
