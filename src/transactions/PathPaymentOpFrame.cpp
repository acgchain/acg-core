...
PathPaymentOpFrame::doApply(Application& app, AbstractLedgerState& ls)
{
    ...
    if (ls.loadHeader().current().ledgerVersion < 8)
    {
        doesSourceAccountExist =
            (bool)acg::loadAccountWithoutRecord(ls, getSourceID());
    }

    if (!bypassIssuerCheck)
    {
        if (!acg::loadAccountWithoutRecord(ls, mPathPayment.destination))
        {
            app.getMetrics()
    ...
    {
        auto destination = acg::loadAccount(ls, mPathPayment.destination);
    ...
    else
    {
        ...
        {
            auto issuer =
                acg::loadAccountWithoutRecord(ls, getIssuer(curB));
            if (!issuer)
            {
                app.getMetrics()
        ...
        auto destLine =
            acg::loadTrustLine(ls, mPathPayment.destination, curB);
        if (!destLine)
        {
            app.getMetrics()
    ...
    for (int i = (int)fullPath.size() - 1; i >= 0; i--)
    {
        ...
        {
            if (!acg::loadAccountWithoutRecord(ls, getIssuer(curA)))
            {
                app.getMetrics()
                    .NewMeter({"op-path-payment", "failure", "no-issuer"},
                              "operation")
    ...
    if (curB.type() == ASSET_TYPE_NATIVE)
    {
        ...
        {
            sourceAccount = acg::loadAccount(ls, getSourceID());
            if (!sourceAccount)
            {
                app.getMetrics()
