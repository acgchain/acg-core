                              ...
                              AccountID const& accountID)
{
    ...
    {
        // this is buggy caching that existed in old versions of the protocol
        auto res = acg::loadAccount(ls, accountID);
        if (res)
        {
    ...
    {
        return acg::loadAccount(ls, accountID);
    }
}
...
TransactionFrame::processFeeSeqNum(AbstractLedgerState& ls)
{
    ...
    if (fee > 0)
    {
        ...
        acg::addBalance(acc.balance, -fee);
...
{
    auto account = acg::loadAccount(ls, accountID);
    if (!account)
    {
                                      ...
                                      SignerKey const& signerKey) const
{
    ...
    {
        auto removed = acg::addNumEntries(header, account, -1);
        assert(removed);
