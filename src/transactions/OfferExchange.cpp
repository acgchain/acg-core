           ...
           std::vector<ClaimOfferAtom>& offerTrail)
{
    ...
    {
        auto accountB = acg::loadAccountWithoutRecord(ls, accountBID);
        if (!accountB)
        {
    ...
    { // entire offer is taken
        ...
        auto accountB = acg::loadAccount(ls, accountBID);
    ...
    if (numSheepSend != 0)
    {
        ...
        if (sheep.type() == ASSET_TYPE_NATIVE)
        {
            auto accountB = acg::loadAccount(lsInner, accountBID);
        ...
        {
            auto sheepLineAccountB =
                acg::loadTrustLine(lsInner, accountBID, sheep);
    ...
    if (numWheatReceived != 0)
    {
        ...
        {
            auto wheatLineAccountB =
                acg::loadTrustLine(lsInner, accountBID, wheat);
              ...
              bool isPathPayment, std::vector<ClaimOfferAtom>& offerTrail)
{
    ...
    if (!acg::loadAccountWithoutRecord(ls, accountBID))
    {
        throw std::runtime_error(
            "invalid database state: offer must have matching account");
    }
    ...
    {
        accountB = acg::loadAccount(ls, accountBID);
    }
                                   ...
                                   : CrossOfferResult::eOfferPartial;
    {
        ...
        {
            sellingWheatOffer.erase();
            accountB = acg::loadAccount(lsInner, accountBID);
