...
{
    auto res = acg::loadAccountWithoutRecord(ls, k);
    if (mustExist)
    {
        REQUIRE(res);
    }
...
{
    LedgerState ls(app.getLedgerStateRoot());
    return (bool)acg::loadAccountWithoutRecord(ls, k);
}
...
{
    LedgerState ls(app.getLedgerStateRoot());
    auto account = acg::loadAccount(ls, k);
                       ...
                       SequenceNumber seq)
{
    ...
    case MANAGE_OFFER_UPDATED:
    {
        LedgerState ls(app.getLedgerStateRoot());
        auto offer =
            acg::loadOffer(ls, source.getPublicKey(), expectedOfferID);
        REQUIRE(offer);
                        ...
                        ManageOfferEffect expectedEffect)
{
    ...
    if (createPassiveOfferResult.code() == MANAGE_OFFER_SUCCESS)
    {
        ...
        {
            LedgerState ls(app.getLedgerStateRoot());
            REQUIRE(!acg::loadOffer(ls, source.getPublicKey(),
                                        expectedOfferID));
        }
        break;
        default:
